...
// スペースで分割する
Vocab* load_words_in_textfile(string &filename, vector<wstring> &dataset){
	...
	for(int epoch = 1;epoch <= max_epoch;epoch++){
		...
		for(int step = 0;step < train_per_epoch;step++){
			...
			if(prev_orders.find(data_index) == prev_orders.end()){
				...
				if(prev_order.size() != sentence_char_ids.size()){
					...
					for(int c_t_i = 0;c_t_i < sentence_char_ids.size();c_t_i++){
						...
						hpylm->remove(sentence_char_ids, c_t_i, n_t);
					}				
				}
			}
			...
			if(prev_order.size() != sentence_char_ids.size()){
				...
				for(int c_t_i = 0;c_t_i < sentence_char_ids.size();c_t_i++){
					int n_t = hpylm->sampleOrder(sentence_char_ids, c_t_i);
					hpylm->add(sentence_char_ids, c_t_i, n_t);
...
int main(int argc, char *argv[]){
	...
	if(argc % 2 != 1){
		...
		for (int i = 0; i < argc; i++) {
			...
			if (string(argv[i]) == "-t" || string(argv[i]) == "--text") {
				...
				model_filename = string(argv[i + 1]);
			}
		}
	}
	...
	// train_vpylm(vocab, dataset, model_filename);
	return 0;
}
